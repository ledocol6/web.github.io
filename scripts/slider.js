const swiper = new Swiper('.swiper', {
  ...
  function initializeSlideLogic() {
    if (window.innerWidth <= 1000) {
      swiper.allowTouchMove = false;
      if (window.innerWidth <= 1000) {
        document.querySelectorAll('.product-card').forEach(card => {
          const imgsContainer = card.querySelector('.card-imgs');
          const bullets = card.querySelectorAll('.bullet');
          let currentIndex = 0;
          let startX = 0; // Начальная точка касания
          let endX = 0; // Конечная точка касания
        
          // Обновляем активный буллет для текущего слайдера
          function updateActiveBullet() {
              bullets.forEach((bullet, index) => {
                  bullet.classList.toggle('bullet-active', index === currentIndex);
              });
          }
        
          // Функция для переключения изображений
          function sliderGo(index) {
              const width = imgsContainer.getBoundingClientRect().width;
              imgsContainer.style.transform = `translateX(-${width * index}px)`; // Перемещение изображений
              currentIndex = index;
              updateActiveBullet();
          }
          ...
          function handleSwipe() {
              ...
              if (Math.abs(diff) > 50) { // Проверяем, что свайп достаточно длинный
                  if (diff < 0 && currentIndex < bullets.length - 1) {
                      // Свайп влево
                      sliderGo(currentIndex + 1);
          ...
          // Наведение на контейнер изображений
          imgsContainer.addEventListener('touchstart', e => {
              swiper.allowTouchMove = false; // Отключаем свайпы внешнего слайдера
              startX = e.touches[0].clientX; // Запоминаем начальную точку касания
          });
        
          imgsContainer.addEventListener('touchmove', e => {
              endX = e.touches[0].clientX; // Обновляем конечную точку в процессе движения
          });
        
          imgsContainer.addEventListener('touchend', () => {
              handleSwipe(); // Вызываем обработку свайпа
              // Включаем свайпы внешнего слайдера
          });
      ...
      document.querySelectorAll('.product-card').forEach(card => {
        const imgsContainer = card.querySelector('.card-imgs');
        const paginator = card.querySelector('.paginator-for-slider');
        const bullets = card.querySelectorAll('.bullet');
        let currentIndex = 0;
        let isHoveringBullet = false; // Флаг для проверки, наводится ли курсор на точку
        ...
        imgsContainer.addEventListener('mouseenter', () => {
            sliderGo(1); // Автоматически переключаем на второй слайд
        });

        // Клик по буллетам
        bullets.forEach((bullet, index) => {
            bullet.addEventListener('mouseenter', () => {
                isHoveringBullet = true;  // Устанавливаем флаг, что курсор на точке
            });

            bullet.addEventListener('mouseleave', () => {
                isHoveringBullet = false;  // Сбрасываем флаг, когда курсор покидает точку
            });

            bullet.addEventListener('click', () => {
                sliderGo(index); // Переключаем на соответствующий слайд при клике на точку
            });
        });

        // Когда курсор покидает весь блок (картинки и точки)
        card.querySelector('.image-cont').addEventListener('mouseleave', () => {
            if (!isHoveringBullet) {
                sliderGo(0);  // Возвращаемся на первый слайд, только если не находимся на точке
            }
        });
